    ...
    stages {
        ...
        stage('Run Application') {
            steps {
                ...
                sh "docker run -d -p 5012:80 -p 5013:443 --name ${CONTAINER_NAME} ${IMAGE_NAME}"
            }
        }
    }
    post {
        ...
        success {
            slackSend(channel: '#scheduling-integration-solution', message: "SUCCESS: The build of ${env.PROJECT_NAME}:${env.BUILD_NUMBER} on branch ${env.TARGET_BRANCH_NAME} succeeded. http://172.20.21.78:5012/")
            script {
                // Prepare a success message
                def message = "Build SUCCESS: ${env.JOB_NAME} #${env.BUILD_NUMBER} - ${env.BUILD_URL}  - ${env.TARGET_BRANCH_NAME} - http://172.20.21.78:5012/"
